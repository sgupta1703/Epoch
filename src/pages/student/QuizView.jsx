...
function TaggedEssay({ text }) {
  ...
  return (
    <div style={{ marginTop: 12 }}>
      ...
      {/* Tagged text */}
      <div style={{
        fontSize: 13, lineHeight: 1.9, background: '#fafaf8',
        border: '1px solid var(--border)', borderRadius: 6,
        padding: '14px 18px', fontFamily: 'var(--font-body)',
      }}>
        {parts.map((part, i) => {
          if (part.type === 'plain') return <span key={i}>{part.text}</span>;
          const s = TAG_STYLES[part.type];
          return (
            <mark key={i} title={s.label} style={{
              background: s.bg, color: s.color,
              borderBottom: `2px solid ${s.border}`,
              borderRadius: 2, padding: '1px 2px', cursor: 'default',
            }}>
              {part.text}
            </mark>
          );
...
export default function QuizView({ user }) {
  ...
  async function fetchAll() {
    ...
    setError('');
    try {
      const [{ classrooms }, { unit }, { quiz }] = await Promise.all([
        getClassrooms(),
        getUnit(unitId),
        getQuiz(unitId),
      ]);
  ...
  if (loading) return (
    <>
      ...
      <div className="app-shell">
        ...
        <main className="page-main"><LoadingSpinner fullPage label="Loading quiz…" /></main>
      </div>
    </>
  );
            ...
            <>
              <div className="quiz-result-score">
                {submission.score !== null ? (
                  <>
                    <div className="quiz-result-score-value" style={{ color: scoreColor(submission.score) }}>
                      {submission.score}%
                    </div>
                    <div className="quiz-result-score-label">
                      Final score · Submitted {new Date(submission.submitted_at).toLocaleDateString()}
                    </div>
                  </>
                ) : (
                  <>
              ...
              {quiz.questions.map((q, i) => {
                ...
                const saResult      = submission.sa_feedback?.find(r => r.question_id === q.id);
                const essayResult   = submission.essay_feedback?.find(r => r.question_id === q.id);
                const isMC    = q.type === 'multiple_choice';
                const isEssay = q.type === 'essay';
                const mcResult  = submission.mc_results?.find(r => r.question_id === q.id);
                const isCorrect = isMC && isMatchingMcAnswer(studentAnswer?.answer, mcResult?.correct_answer || q.correct_answer);
                ...
                return (
                  <div key={q.id} className="quiz-student-question">
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      ...
                      {isMC && mcResult && (
                        <span style={{
                          ...
                          color:      isCorrect ? '#2a7a2a' : '#c0392b',
                          background: isCorrect ? '#eaf6ea' : '#fdecea',
                      ...
                      {!isMC && !isEssay && saResult?.score !== null && saResult?.score !== undefined && (
                        <span style={{ fontSize: 13, fontWeight: 600, color: scoreColor(saResult.score) }}>
                          {saResult.score}%
                        </span>
                      )}

                      {isEssay && essayResult?.score !== null && essayResult?.score !== undefined && (
                        <span style={{ fontSize: 13, fontWeight: 600, color: scoreColor(essayResult.score) }}>
                          {essayResult.score}%
                        </span>
                      )}
                    </div>
                    ...
                    <div
                      className="quiz-student-question-text quiz-student-question-text--markdown"
                    ...
                    {/* MC: show correct if wrong */}
                    {isMC && !isCorrect && mcResult?.correct_answer && (
                      <div style={{ fontSize: 13, color: '#2a7a2a', background: '#eaf6ea', padding: '8px 12px', borderRadius: 4, marginBottom: 0 }}>
                        Correct answer: <strong>{mcResult.correct_answer}</strong>
                      </div>
                    )}

                    {/* SA: feedback */}
                    {!isMC && !isEssay && saResult && (
                      <div style={{
                        fontSize: 13, color: 'var(--ink)', background: '#fff',
                        border: '1px solid var(--border)',
                        borderLeft: `3px solid ${scoreColor(saResult.score ?? 0)}`,
                        padding: '8px 12px', borderRadius: '0 4px 4px 0', lineHeight: 1.6,
                      }}>
                        💬 {saResult.feedback}
                      </div>
                    )}

                    {/* Essay: breakdown + feedback + tagged response */}
                    {isEssay && essayResult && (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                          ...
                          <div className="essay-score-breakdown">
                            {Object.entries(essayResult.breakdown).map(([key, val]) => (
                              <div key={key} className="essay-score-cell">
                                ...
                                <div className="essay-score-value" style={{ color: scoreColor(val * 4) }}>
                                  {val}<span className="essay-score-denom">/25</span>
                                </div>
                              </div>
                            ))}
                          </div>
                        ...
                        {essayResult.feedback && (
                          <div style={{
                            fontSize: 13, color: 'var(--ink)', background: '#fff',
                            border: '1px solid var(--border)',
                            borderLeft: `3px solid ${scoreColor(essayResult.score ?? 0)}`,
                            padding: '10px 14px', borderRadius: '0 4px 4px 0', lineHeight: 1.7,
                          }}>
                            💬 {essayResult.feedback}
                          </div>
                        )}
                        ...
                        {essayResult.tagged_response && (
                          <div>
                            <p style={{ fontSize: 12, fontWeight: 500, color: 'var(--muted)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 6 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20 }}>
                ...
                <div style={{ height: 4, width: 160, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                  <div style={{
                    ...
                    background: 'var(--rust)', borderRadius: 2, transition: 'width 0.2s',
                  }} />
                </div>
              </div>
                ...
                <div key={q.id} className="quiz-student-question">
                  ...
                  {q.type === 'essay' && (
                    <>
                      ...
                      <textarea
                        className="quiz-essay-answer"
                        value={answers[q.id] || ''}
                        onChange={e => handleAnswer(q.id, e.target.value)}
                        placeholder="Write your essay response here. Include a clear thesis, support it with specific historical evidence, explain its significance through analysis, and address a counterargument…"
                        rows={12}
                      />
