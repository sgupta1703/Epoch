...
// ── Tagged essay renderer (same as QuizView) ──────────────────
    ...
    <div style={{ marginTop: 12 }}>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginBottom: 12 }}>
        {Object.entries(TAG_STYLES).map(([type, s]) => (
          <span key={type} style={{ fontSize: 11, fontWeight: 500, padding: '3px 8px', borderRadius: 3, background: s.bg, color: s.color, border: `1px solid ${s.border}`, letterSpacing: '0.04em' }}>
            {s.label}
          </span>
        ))}
      </div>
      <div style={{ fontSize: 13, lineHeight: 1.9, background: '#fafaf8', border: '1px solid var(--border)', borderRadius: 6, padding: '14px 18px', fontFamily: 'var(--font-body)' }}>
        {parts.map((part, i) => {
          ...
          return (
            <mark key={i} title={s.label} style={{ background: s.bg, color: s.color, borderBottom: `2px solid ${s.border}`, borderRadius: 2, padding: '1px 2px', cursor: 'default' }}>
              {part.text}
            </mark>
          );
        })}
...
export default function AssignmentView({ user }) {
  ...
  async function fetchAll() {
    ...
    setError('');
    try {
      const [{ classrooms }, { unit }, { assignment }] = await Promise.all([
        getClassrooms(),
        getUnit(unitId),
        getAssignment(unitId),
      ]);
  ...
  if (loading) return (
    <>
      ...
      <div className="app-shell">
        ...
        <main className="page-main"><LoadingSpinner fullPage label="Loading assignment…" /></main>
      </div>
    </>
  );
        ...
        <main className="page-main">
          ...
          {!assignment ? (
            ...
            <>
              ...
              {/* Sources (collapsed, still readable) */}
              {sources.length > 0 && (
                <div style={{ marginBottom: 32 }}>
                  ...
                  {sources.map((s, i) => (
                    <div key={i} className="assignment-source-block">
                      <button
                        className="assignment-source-toggle"
                        onClick={() => setExpandedSource(expandedSource === i ? null : i)}
                      >
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span className={`assignment-student-badge assignment-student-badge--${s.source_type}`}>
                            {s.source_type === 'primary' ? 'Primary' : 'Secondary'}
                          </span>
                          <span className="assignment-source-toggle-title">{s.title}</span>
                        </div>
                        <span style={{ fontSize: 18, color: 'var(--muted)', lineHeight: 1 }}>
                          {expandedSource === i ? '−' : '+'}
                        </span>
                      </button>
                      {expandedSource === i && (
            ...
            <div className="assignment-layout">
              ...
              <div className="assignment-sources-panel">
                ...
                {sources.map((s, i) => (
                  <div key={i} className="assignment-source-block">
                    <button
                      className={`assignment-source-toggle ${expandedSource === i ? 'assignment-source-toggle--active' : ''}`}
                      onClick={() => setExpandedSource(expandedSource === i ? null : i)}
                    >
                    ...
                    {q.type === 'essay' && (
                      <>
                        ...
                        <textarea
                          ...
                          value={answers[q.id] || ''}
                          onChange={e => handleAnswer(q.id, e.target.value)}
                          placeholder="Write your essay response here. Use the sources above as evidence — cite specific details, make a clear argument, and address a counterargument…"
                          rows={12}
                        />
