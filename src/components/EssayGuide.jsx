...
import { evaluateEssayGuideOutline, chatWithEssayGuideApi } from '../api/assistant';
...
async function chatWithGuide(question, essay, messages) {
  const { reply } = await chatWithEssayGuideApi(question, essay, messages);
  return reply;
}
...
export default function EssayGuide({ isOpen, onClose, question, essayDraft }) {
  ...
  const [chatMessages, setChatMessages] = useState([
    { role: 'assistant', content: "Hi! I'm your Essay Guide. Ask me anything about your essay - your thesis, evidence, structure, or what the AP rubric looks for." },
  ]);
  ...
  async function handleEvaluate() {
    ...
    try {
      ...
      setEvalError('Failed to evaluate. Check your connection and try again.');
    } finally {
      setEvaluating(false);
    }
  }
    ...
    <>
      ...
      >
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <button onClick={onClose} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 20, color: 'var(--muted)', lineHeight: 1, padding: 4 }}>x</button>
        </div>
              ...
              <div key={f.key}>
                ...
                <textarea
                  ...
                  onChange={e => {
                    ...
                    setFeedback(fb => (fb ? { ...fb, [f.key]: null } : fb));
                  }}
